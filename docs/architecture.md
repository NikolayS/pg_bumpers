...
1. **WALL — native Postgres roles/RLS (hardened).** The WALL is **agent-scoped** (issue #108 — the default `deploy/sql/10_hardened_role.sql` constrains `pgb_agent`/`pgb_applier` and does **not** `REVOKE … FROM PUBLIC`, so it is safe on a shared BYO DB): the agent role is `NOINHERIT`, member of nothing, no superuser, no write grant (default-deny on data), SELECT-whitelisted; every `pg_*` predefined role (`pg_read_all_data`, …) + `REPLICATION` revoked; agent `EXECUTE` on existing `public` functions revoked; `search_path` pinned; `dblink`/`postgres_fdw`/`COPY … PROGRAM`/`lo_import`/`lo_export`/`pg_read_file` denied structurally (superuser/predefined-role gated). **Honest residual (the agent-only default does NOT revoke `PUBLIC`):** on a shared BYO DB `PUBLIC`'s default function-`EXECUTE` (on funcs created after apply), `TEMP` (`CREATE TEMP TABLE`), and the in-DB large-object write built-ins (`lo_*`) remain at the DB level — none can be denied by an agent-scoped revoke (the **B-lo** residual). That write class is contained **through the proxy** by the M2a fail-closed read classifier (layer 2, #114/#115 — `SELECT lo_create()`/non-allowlisted or qualified fn / custom operator / qualified cast / `FOR`-lock all classify `NotRead` → Blocked) and **direct-to-DB** by the layer-0 network boundary; the opt-in `deploy/sql/21_public_lockdown.sql` restores the DB-level `… FROM PUBLIC` denies for a dedicated DB (see `SPEC.amendments.md` A-M2, `KNOWN_BYPASSES.md` B-lo). "Not superuser" is insufficient. This is the un-foolable backstop.
...
Since **M2a (#114/#115)** the classifier is a **fail-closed allowlist gate** — a `SELECT` is `Read` only if EVERY function it references is on the curated read-safe list, so `pg_sleep`/`nextval`/`setval`/`lo_*`/`pg_read_file`/custom-operator/qualified-cast writes classify `NotRead` → **Blocked at the proxy floor** (the residual is the exotic bare-cast, `KNOWN_BYPASSES.md` B9). The un-foolable backstops behind it — WALL role, `statement_timeout`, byte/row cutoff — remain fail-closed and are exercised end-to-end against live PG18 in [`crates/proxy/tests/proxy_it.rs`](../crates/proxy/tests/proxy_it.rs), including `SELECT pg_sleep(5)` blocked at the read-only gate and the marquee `COMMIT; DROP SCHEMA public CASCADE` block (schema intact).
