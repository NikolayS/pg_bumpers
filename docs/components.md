...
- **`enforce`** — the **pure, synchronous frontend-frame gate** `Enforcement::gate` → `GateDecision::{Allow, Reject, Block}` (`RejectKind::{SimpleQuery, Copy}`). Two layers: the tag gate (extended-only) rejects 'Q'/Copy*; the SQL gate classifies `Parse` text read-only. The **marquee block** is proven here as a plain call — `COMMIT; DROP SCHEMA public CASCADE` over simple-query is `Reject(SimpleQuery)`, and the same text smuggled into one `Parse` body is `Block(stacked_statement)` (belt-and-suspenders). Post-M2a (#114/#115) the classifier fail-closes on ANY non-allowlisted function, so `SELECT pg_sleep(30)`/`SELECT nextval(…)`/`SELECT lo_create(…)` now classify `NotRead` → Blocked (the read path is an allowlist, not a denylist); `statement_timeout` + the byte/row cutoff remain the un-foolable budget backstops behind it.
...
**The six enforcement hooks** (lib.rs module docs): (1) extended-protocol-only; (2) read-only classification; (3) byte/row mid-stream cutoff; (4) `statement_timeout` injection; (5) fail-closed on any parse/enforcement uncertainty; (6) audit of every statement incl. rejects. Since M2a (#114/#115) the classifier is a **fail-closed allowlist gate** — a `SELECT` is `Read` only if EVERY function it references is on the curated read-safe list, so `lo_*`/`setval`/`nextval`/`pg_read_file`/`pg_sleep`/custom-operator/qualified-cast writes are `NotRead` → Blocked at the proxy floor (the residual is the exotic bare-cast, `KNOWN_BYPASSES.md` B9). The un-foolable backstops behind it remain the WALL role, `statement_timeout`, and the byte/row cutoff.
...
- **Layer 1 — hardened role** (`deploy/sql/10_hardened_role.sql`, idempotent, also mirrored under `deploy/init/`): the **agent-only default** (issue #108 — it constrains `pgb_agent`/`pgb_applier` and **NEVER mutates `PUBLIC`**, so it is safe on a shared BYO DB). The `pgb_agent` role is `LOGIN, NOSUPERUSER, NOINHERIT, NOCREATEDB, NOCREATEROLE, NOREPLICATION, NOBYPASSRLS`, **member of nothing** (every `pg_*` predefined role revoked, incl. `pg_read_all_data`/`pg_execute_server_program`/`pg_read_server_files`), no CREATE on `public` (agent-scoped revoke; on **PG14** `PUBLIC` still has CREATE — a documented residual), **no DML write grant on any relation** (default-deny on data), and **SELECT only on an explicit whitelist** (`public.allowed_read` granted; `public.secret_data` not). Honest residual on a shared DB (the default does NOT globally revoke `PUBLIC`): the agent retains, at the DB level, `PUBLIC`'s default function-`EXECUTE`, `TEMP` (`CREATE TEMP TABLE`), and the in-DB large-object write built-ins (`lo_*`). That write class is contained **through the proxy** by the M2a fail-closed read classifier (`crates/pgwire`, #114/#115 — a `SELECT lo_create()`/`lowrite()`/non-allowlisted or qualified fn / custom operator / qualified cast / `FOR`-lock all classify `NotRead` → Blocked), and **direct-to-DB** by the Layer-0 network boundary; the opt-in `21_public_lockdown.sql` restores the DB-level `… FROM PUBLIC` revokes for a dedicated DB (see `SPEC.amendments.md` A-M2, `KNOWN_BYPASSES.md` B-lo). Other honest caveats are spelled out in-file: the role-level `search_path` pin is best-effort defense-in-depth (the authoritative pin is the proxy), and the WALL's real guarantee does not rely on `search_path`. `deploy/test/wall_matrix.sh` asserts each deny (and each documented residual) by attempting it.
